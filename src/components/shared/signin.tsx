...
const Signin = ({ callback_url }: { callback_url?: string }) => {
	...
	return (
		<section className="flex items-center justify-center w-full px-4">
			<form className="p-6 xs:p-10 w-full max-w-[350px] flex flex-col justify-between items-center gap-2.5 bg-white rounded text-black">
				...
				<button
					className="w-full h-14 justify-center flex py-3 px-4 text-sm align-middle  border items-center rounded text-999 bg-[#F4F4F5] transition duration-150 ease hover:bg-gray-200 gap-3"
					onClick={(e) => {
						e.preventDefault();

						try {
							signIn("google", { callbackUrl, redirect: false }); // Calls the signIn function with "google" provider
						} catch (error) {
							toast.error("حصل خطأ ما!، الرجاء إعادة المحاولة");
						}
					}}
				>
